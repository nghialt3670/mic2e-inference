...
class ObjectClearPipeline(
    ...
    def __init__(
        ...
        self.register_to_config(
            apply_attention_guided_fusion=apply_attention_guided_fusion
        )
        ...
        **kwargs,
        ...
        if pretrained_model_name_or_path == "jixin0101/ObjectClear":
            ...
            safetensor_path = os.path.join(
                pretrained_model_name_or_path, sub_folder, filename
            )
    ...
    def unet_store_cross_attention_scores(
        self, unet, attention_scores, applicable_layers=None
    ):
        import types
    ...
    def unet_restore_attention_processor(self, unet, original_state):
        ...
        for name, module in unet.named_modules():
            if (
                isinstance(module, Attention)
                and "attn2" in name
                and name in original_state
            ):
                module.get_attention_scores = original_state[name][
    ...
    def resize_attn_map_divide2(self, attn_map, mask, fuse_index):
        ...
        attn_map = F.interpolate(
            attn_map, size=(H, W), mode="bilinear", align_corners=False
        )
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if (
                    i == len(timesteps) - 1
                    and self.config.apply_attention_guided_fusion
                ):
                    self.unet, self.original_state = (
                        self.unet_store_cross_attention_scores(
                ...
                if self.config.apply_attention_guided_fusion:
                    ...
                    if (
                        ...
                        attn_key, attn_map = next(
                            iter(self.cross_attention_scores.items())
                        )
                        attn_map = self.resize_attn_map_divide2(
                            attn_map, mask, fuse_index
                        )
                        init_latents_proper = image_latents
                        if self.do_classifier_free_guidance:
        ...
        if output_type == "pil" and attn_map is not None:
            for i in range(len(attn_map)):
                attn_np = attn_map[i].mean(dim=0).cpu().numpy() * 255.0
                attn_pil = PIL.Image.fromarray(attn_np.astype(np.uint8)).convert("L")
                attn_pils.append(attn_pil)

            original_pils = self.image_processor.postprocess(
                init_image, output_type="pil"
            )
            ...
            for i in range(len(generated_pils)):
                ...
                fused_np = attention_guided_fusion(
                    np.array(ori_pil), np.array(gen_pil), np.array(attn_pil)
                )
                fused_pil = PIL.Image.fromarray(fused_np.astype(np.uint8)).resize(
                    ori_pil.size
                )
